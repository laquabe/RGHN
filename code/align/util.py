...
import pickle
from .input import read_dbp15k_input, read_openea_input
...
def gcn_load_data(input_folder, is_two=False, is_three=False, is_four=False):
    ...
    kg1, kg2, sup_ent1, sup_ent2, ref_ent1, ref_ent2, total_tri_num, total_e_num, total_r_num, rel_id_mapping, rel1_list, rel2_list = \
         read_openea_input(input_folder)
    # rel_id_mapping
    ...
    # enhance_triples: replace r1 to r2, eg:as (h1, r1, t1) if h1 and t1 in seed,replace r1 with r2 as (h1, r2, t1) in triple1
    ...
    # remove triples which is not in link, to reduce noisy
    ...
    return adj, kg1, kg2, sup_ent1, sup_ent2, ref_ent1, ref_ent2, total_tri_num, \
           total_e_num, total_r_num, rel_id_mapping, rel_ht_dict, rel1_list, rel2_list
...
def relation_adj_list(kg1, kg2, adj_number, all_rel_num, all_ent_num, linked_ents, rel_id_mapping):
    ...
    adj_len = list()
    for i in range(adj_number):
        pos = np.array(edge[sort_edge_length[i]])   #get edge position
        row, col = np.transpose(pos)
        data = np.ones(shape=int(edge_length[sort_edge_length[i]]))
    ...
    return adj_list, list(sort_edge_length), edge

def relation_adj_list_augment(triple, adj_number, all_rel_num, all_ent_num, rel_id_mapping):
    rel_dict = rel_id_mapping
    adj_list = list()
    triple_list = triple
    edge = dict()
