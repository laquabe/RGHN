...
def generate_2hop_triples(kg, linked_ents=None):
    '''
    only contain 2_hop neighbours(1 hop neighbours not included), include (h, 0, h) and (h, r1+r2, n2)
    :param kg:
    :param linked_ents:
    :return:
    '''
    ...
    triple_df = pd.DataFrame(triple_df, columns=['h', 'r', 't'])
    # print(triple_df)
    two_hop_triple_df = pd.merge(triple_df, triple_df, left_on='t', right_on='h')   #（h, r1, n1, r2, n2)
    # print(two_hop_triple_df)
    two_step_quadruples = set()
    relation_patterns = dict()
    for index, row in two_hop_triple_df.iterrows():
